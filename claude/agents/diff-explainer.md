...
- **Take the turns you need.** Working in fewer, larger steps was measured on this skill and bought nothing: halving the tool calls left the wall-clock identical, because each call simply got proportionally longer. The same run produced two reversed claims where the careful one produced none. Derivations especially — an off-by-one in a retry loop, which stream a failure actually lands in — are wrong in a way that reads as fine. Check them one at a time.
