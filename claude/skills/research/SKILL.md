...
2. Read the output template from `assets/output-template.md` (relative to this skill's directory) with the Read tool. Write the research document using the Write tool, populating each section with the corresponding data from Phase 2's JSON output.
...
Actions:
1. research-planner がスコープを決定（topic: 認証フロー, scope: focused）
2. codebase-investigator が関連ファイルを深掘り調査
3. ユーザーに調査結果サマリーを提示
4. 承認後 `research-auth-flow.md` として出力
